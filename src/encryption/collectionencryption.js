...
//port here to resolve when testing
if (Match === undefined) {
  var Match = {
    Optional: function() {}
  };
...
CollectionEncryption = function(collection, fields, config) {
  ...
  /*  //!fixme: apply react mongoose hook library
...
_.extend(CollectionEncryption.prototype, {
  ...
  _initSchema: function() {
    ...
    Meteor.call("initEncryptionSchema", self.collection._name, self.getEncryptedFieldKey());
  },
  /**
  ...
  _decryptDoc: function(doc) {
    ...
    doc = EncryptionUtils.decryptDoc(doc, self.fields, self.principalName, self.getEncryptedFieldKey());
  ...
  _listenToUpdates: function() {
    ...
    self.collection.before.update(function(userId, doc, fieldNames, modifier) {
      ...
      modifier = self.startDocUpdate(userId, doc, fieldNames, modifier);
    });
  },
  /**
  ...
  /**
   * starts the encryption of a document by
   *   removing the content that should be encrypted
  ...
  finishDocEncryption: function(doc) {
    ...
    // get encrypted doc
    var encryptedDoc = EncryptionUtils.encryptDocWithId(doc, self.fields, self.principalName, documentKey);
   ...
   */
  shareDocWithUser: function(docId, userId, externalDoc) {
    var self = this;
    EncryptionUtils.shareDocWithUser(docId, self.principalName, userId, externalDoc);
  },
  /**
